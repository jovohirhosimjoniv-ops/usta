...
export const Route = createFileRoute("/xabarlar")({
  head: () => ({
  ...
  useEffect(() => {
    const fetchBuyurtmalar = async () => {
      try {
        ...
        const response = await api.get("/buyurtmalar/");
  ...
  useEffect(() => {
    try {
      ...
      if (savedUser) {
        ...
        setUser({
          ism: parsed.ism || parsed.username || "Mijoz",
          familiya: parsed.familiya || "",
          shahar: joylashuv,
    ...
    >
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold">Mening buyurtmalarim ({buyurtmalar.length})</h2>
        </div>
        ...
        {loadingBuyurtmalar ? (
          ...
          <div className="max-h-[300px] overflow-y-auto pr-2">
            <div className="grid gap-4 sm:grid-cols-2">
              {buyurtmalar.map((item) => {
                ...
                // Usta ma'lumotlarini ajratib olish
                const ustaId = item.usta || item.usta_details?.id;
                const ustaIsm = item.usta_details?.ism || item.usta_name || item.usta_details?.username || "Usta";
                const ustaRasm = getImageUrl(item.usta_details?.rasm || item.usta_details?.profile?.rasm);
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <h3 className="font-bold text-foreground">
                            {item.xizmat_turi || item.xizmat || "Xizmat ko'rsatilgan"}
                          </h3>
                        </div>
                      ...
                      <div className="space-y-1.5 text-sm text-muted-foreground">
                        {/* Usta ismi va uning ma'lumoti */}
                        <p className="flex items-center gap-2 font-medium text-foreground">
                          <UserCheck className="size-4 text-primary shrink-0" />
                          <span>Usta: <strong className="text-foreground">{ustaIsm}</strong></span>
                        </p>
                    ...
                    {/* Chat tugmasi va Bekor qilish tugmasi */}
                    <div className="mt-4 border-t pt-3 flex items-center justify-between gap-2">
                      ...
                      >
                        ...
                        {cancelingId === item.id ? "O'chirilmoqda..." : "Bekor qilish"}
                      </Button>
                    </div>
                  </div>
                );
          ...
          <div className="rounded-2xl border border-dashed p-8 text-center text-muted-foreground space-y-3">
      ...
      <section className="space-y-4">
        ...
        {saqlanganUstalar.length > 0 ? (
          ...
          <div className="rounded-2xl border border-dashed p-8 text-center text-muted-foreground">
            Siz hali hech qaysi ustani saqlamagansiz. Ustalar ro'yxatidagi
